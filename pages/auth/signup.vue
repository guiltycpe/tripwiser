    ...
    <div class="w-full max-w-md">
      <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm animate-fade-in">
        <div class="text-center mb-8">
          ...
          <h1 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">{{ t.auth.signup.title }}</h1>
          <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">{{ t.auth.signup.subtitle }}</p>
        </div>
        ...
        <form @submit.prevent="handleSignup" class="space-y-5">
          <div>
            <label for="fullName" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ t.auth.signup.fullName }}</label>
            <input
          ...
          <div>
            <label for="email" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ t.auth.signup.email }}</label>
            <input
          ...
          <div>
            <label for="password" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ t.auth.signup.password }}</label>
            <input
              id="password" v-model="password" type="password" required minlength="6"
              class="block w-full px-4 py-2.5 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 hover:border-slate-300 dark:hover:border-slate-600"
              :placeholder="t.auth.signup.passwordPlaceholder"
            />
            <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">{{ t.auth.signup.passwordHint }}</p>
          </div>

          <div class="flex items-start">
            <input id="terms" v-model="acceptedTerms" type="checkbox" required class="mt-1 rounded border-slate-300 dark:border-slate-600 text-teal-600 focus:ring-teal-500" />
