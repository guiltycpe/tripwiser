  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <div v-else-if="error" class="flex items-center justify-center min-h-screen">
      <div class="text-center max-w-md">
        ...
        <h1 class="text-xl font-semibold text-slate-900 mb-2">Trip not found</h1>
        <p class="text-sm text-slate-500 mb-6">This shared link may be invalid or the trip has been removed.</p>
    ...
    <div v-else-if="trip" class="max-w-5xl mx-auto px-4 py-8">
      ...
      <div class="text-center mb-10">
        <div class="inline-flex items-center gap-2 bg-teal-50 text-teal-700 text-xs font-medium px-3 py-1.5 rounded-lg border border-teal-100 mb-4">
          <Icon name="heroicons:share-20-solid" class="h-3.5 w-3.5" />
          Shared Itinerary
        </div>

        <h1 class="text-3xl md:text-4xl font-semibold text-slate-900 mb-2">
      ...
      <div v-if="trip.itinerary?.trip_summary" class="grid grid-cols-3 gap-4 mb-10">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4 text-center">
          ...
          <div class="text-xs text-slate-500 font-medium">Per Person</div>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4 text-center">
          <div class="text-xl font-semibold text-slate-700">${{ Math.round(trip.itinerary.trip_summary.total_estimated_cost_usd / trip.itinerary.trip_summary.duration_days) }}</div>
          <div class="text-xs text-slate-500 font-medium">Per Day</div>
        </div>
      </div>

      <!-- Itinerary -->
      <div v-if="trip.itinerary?.itinerary_sections" class="space-y-6">
        <div v-for="section in trip.itinerary.itinerary_sections" :key="section.section_title" class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div class="p-6 space-y-6">
            <div v-for="day in section.daily_plans" :key="day.day">
              <h3 class="text-base font-semibold text-slate-900 mb-1">Day {{ day.day }}: {{ day.title }}</h3>
              <span class="inline-block text-xs font-medium bg-teal-50 text-teal-700 px-2.5 py-1 rounded-lg border border-teal-100 mb-3">{{ day.daily_pace }}</span>

              <div class="space-y-3 ml-3 border-l-2 border-slate-200 pl-4">
                <div v-for="act in day.activities" :key="act.description" class="flex gap-3">
                  ...
                  <div class="flex-1">
                    <p class="text-sm font-medium text-slate-900">{{ act.description }}</p>
                    <p v-if="act.notes" class="text-xs text-slate-500 mt-0.5">{{ act.notes }}</p>
      ...
      <div class="text-center mt-12 py-8 border-t border-slate-200">
        <p class="text-sm text-slate-500 mb-4">Want to create your own personalized itinerary?</p>
